...
#order by precip
#precip_ordered_sept_may <- combined[order(combined$precip_sept_may_mm),]
#top 5 driest years for precip_nov_apr
#precip_ordered_sept_may[1:5,] 
#precip_ordered_sept_may[105:109,]

#order by precip
temp_ordered_nov_apr <- combined[order(combined$temp_nov_apr_celsius),]
#top 5 driest years for precip_nov_apr
coldest_ordered <- temp_ordered_nov_apr[1:5,] 
warmest_ordered <- temp_ordered_nov_apr[105:109,] 
coldest_ordered <- as.data.frame(cbind(coldest_ordered$year, coldest_ordered$temp_nov_apr_celsius, coldest_ordered$growth_index))
colnames(coldest_ordered) <- c("year", "temp", "growth_index")
coldest_ordered <- coldest_ordered[order(coldest_ordered$temp),]

warmest_ordered <- as.data.frame(cbind(warmest_ordered$year, warmest_ordered$temp_nov_apr_celsius, warmest_ordered$growth_index))
colnames(warmest_ordered) <- c("year", "temp", "growth_index")
warmest_ordered <- warmest_ordered[order(-warmest_ordered$temp),]

#order by precip
#temp_ordered_apr_oct <- combined[order(combined$temp_apr_oct_celsius),]
#top 5 driest years for precip_nov_apr
#temp_ordered_apr_oct[1:5,] 
#temp_ordered_apr_oct[105:109,] 

#-----

ggplot(combined, aes(combined$temp_apr_oct,combined$precip_nov_apr)) + geom_text(aes(label=year)) + ggtitle("Petal and sepal length of iris")
...
plot(combined$growth_index, combined$precip_nov_apr, main = "Precipitation (Nov-Apr) vs. Blue Oak Growth Index \n (black = 1896-1950, red = 1950-2004)",ylab = "Precipitation (mm)", xlab = "Growth Index", col = combined$Groups)

mod2<-lm(combined$precip_nov_apr~combined$growth_index)
abline(coefficients(mod2), lwd=2, lty=2, col="red")
correlation <- cor(combined$precip_nov_apr, combined$growth_index,method="pearson")


text(.65, 40, paste("pearson's correlation =", round(correlation, 4),  "\n p-value =", round(summary(mod2)$coefficients[,4][2], 17)))
...
#--Warmest, Coldest, Wettest, Driest years - anomalies for monthly groupings 1981-2010

precip_ordered <- combined[order(combined$precip_nov_apr),]
